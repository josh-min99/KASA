...
# 패널 C: 재하중 구간(3.5시간)만 다른 대비를 포함해 넷을 나란히 본다.
# 순서는 계획서 표 5 와 맞춘다 (CI가 0을 배제한 유전자 수 내림차순).
...
def main():
    ...
    fig.text(0.052, -0.055,
        ...
        "95% 신뢰구간이며 다중검정 보정을 하지 않았다. 15개 × 4개 대비에서 우연히 0 을 배제하는 개수는 "
        "대비당 0.75개가 기대되므로, 아래의 개수는 검정 결과가\n"
        "아니라 기술 통계로만 읽어야 한다. 유전자 순서는 결과가 아니라 시계 회로의 기능 구획으로 "
        "고정하였다. (A)에는 대조군이 서로 다른 세 대비만 표시하였다.\n"
        "네 번째 대비는 처리군끼리의 비교여서 축이 달라지므로 (C)에만 넣었다.\n"
        "(B) 별표를 붙인 Arntl 과 Per2 는 1단계 재현에서 분석 전에 지정한 유전자다"
        "(Life 2020, doi:10.3390/life10090196). 나머지 13개는 탐색 대상이다. Arntl 은 재현되나\n"
        "Per2 는 비행에서 +0.04(d=0.33)로 재현되지 않으며, HLU 에서는 오히려 +0.27(d=1.39)로 "
        "선행연구가 보고한 방향과 반대다.\n"
        "(C) † HLU 군은 12일째 현수 상태에서, HLU+재하중 군은 현수를 푼 뒤 3.5시간을 더 두고 "
        "희생시켰다(STS-108 의 착륙~희생 간격을 맞추기 위한 원저자의 설계).\n"
        "따라서 이 대비에서는 3.5시간의 시간 경과와 하중 재부하가 함께 변하며, 시각 차이만의 "
        "대조가 아니다.\n\n"
        "읽는 법. 우주비행에서 신뢰구간이 0 을 배제하는 유전자는 5개다"
        # 유니코드 마이너스(U+2212)는 이 폰트에서 두부로 깨진다. ASCII 하이픈을 쓴다.
        "(Arntl +0.46, Cry2 +0.61, Nr1d1 +0.37, Per1 +1.58, Hlf -0.21 — 마지막 하나는 방향이 반대다).\n"
        "HLU 에서는 0개다. 그런데 두 대비의 |군간 차이| 중앙값은 0.215 와 0.182 로, 차이가 0.033 에 "
        "불과하다. 이는 두 대비의 신뢰구간 폭(0.64, 0.82)의 4~5% 수준으로\n"
        "해상도 아래에 있다. 즉 5 대 0 이라는 갈림은 효과크기의 차이로 설명되지 않는다. "
        "군당 n=4~5 에서는 '유의한 clock 유전자가 몇 개인가'가 처리보다\n"
        "군내 분산에 좌우된다. 실제로 이 연구에서 clock 신호가 가장 많이 잡힌 대비는 우주비행이 아니라 "
        "지상 실험인 HLU+재하중 vs 정상하중(8개)이다.\n"
        "효과가 0 일 때 잡음만으로 기대되는 |차이| 중앙값과 비교하면 네 대비는 각각 3.53, 2.45, 1.51, "
        "0.99 배다. 맨 아래 대비는 잡음과 구별되지 않으므로\n"
        "3.5시간 구간의 효과 유무를 어느 쪽으로도 판정할 수 없다.\n"
        "여기에 더해 모든 군이 단일 시점 채취이므로 관측된 차이가 진폭 변화인지 위상 이동인지 구별할 수 "
        "없다. 교란을 모두 제거해도 남는 이 두 한계는\n"
        "데이터를 더 고르거나 더 잘 골라서 해결되지 않으며, 충분한 표본과 연속 기록을 요구하는 직접적 "
        "근거다.",
